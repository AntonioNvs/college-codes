...
def initialise_players(team,teamname,params,GKid):
    ...
    for p in player_ids:
        ...
        team_player = player(p,team, "1" if int(p) < 11 else "0",params,GKid)
...
def generate_pitch_control_for_event(event_id, events, tracking_home, tracking_away, params, GK_numbers, field_dimen = (106.,68.,), n_grid_cells_x = 50, offsides=True):
    ...
    if pass_team==1.0:
        attacking_players = initialise_players(tracking_home.loc[event_id],'Home',params,GK_numbers[0])
        defending_players = initialise_players(tracking_away.loc[event_id],'Away',params,GK_numbers[1])
    elif pass_team==0.0:
        defending_players = initialise_players(tracking_home.loc[event_id],'Home',params,GK_numbers[0])
        attacking_players = initialise_players(tracking_away.loc[event_id],'Away',params,GK_numbers[1])
    else:
