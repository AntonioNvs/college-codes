...
class EvalVisitor(Visitor):
    ...
    def visit_and(self, exp, env):
        val_left = exp.left.accept(self, env)
        val_right = exp.right.accept(self, env)
        if type(val_left) == type(True) and type(val_right) == type(True):
    ...
    def visit_neg(self, exp, env):
        val_exp = exp.exp.accept(self, env)
        if type(val_exp) == type(1):
            return -val_exp
        else:
            sys.exit("Type error")
    ...
    def visit_ifThenElse(self, exp, env):
        ...
        if type(val_cond) == type(True):
            return exp.e0.accept(self, env) if val_cond else exp.e1.accept(self, env)
    ...
    def visit_app(self, exp, env):
        ...
        if isinstance(fval, Function):
            val_param = exp.actual.accept(self, env)
            new_env = dict(fval.env)
            new_env[fval.formal] = val_param

            if getattr(fval, "name", None) is not None:
