...
def derivadas_regressao_reg(theta, X, y, b):
    return -2 * ((y - X @ theta) @ X + b*theta)
...
def gd(theta, d_fun, X, y, beta = 0.1, lambda_=0.0001, tol=0.00001, max_iter=10000):
    theta = theta.copy()
    #print('Iter {}; theta = '.format(0), theta)
    old_err_sq = np.inf
    i = 0
...
theta_reg = np.ones(5)
theta_reg = gd(theta, derivadas_regressao, X_train, y_train)
...
assert_equal(np.round(r2_train,2), np.round(0.79,2))
assert_equal(np.round(r2_test,2), np.round(0.722,2))
...
assert_equal(np.round(r2_train,3), np.round(0.7903,3))
assert_equal(np.round(r2_test,3), np.round(0.7235,3))
...
assert_equal(np.round(r2_train,2), np.round(0.7878,2))
assert_equal(np.round(r2_test,2), np.round(0.7158,2))
...
# exercício opcional
get_ipython().system('jupyter nbconvert --to python Lista12.ipynb')
